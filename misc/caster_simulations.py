...
SEQ_LEN = int(2e5) if len(sys.argv) < 3 else int(sys.argv[2])  # 2e7
MUT_RATE = 5e-8
...
tree_list = dendropy.TreeList.get(path="1", schema="newick")
rep = 0
for t in tree_list:
    # rep += 1
    ...
    ts = msprime.sim_ancestry(samples=samples, ploidy=1, demography=demography, recombination_rate=REC_RATE, sequence_length=SEQ_LEN, random_seed=(hash(1) % 100000))  # , model="smc_prime")
